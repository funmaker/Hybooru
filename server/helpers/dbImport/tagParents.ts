...
export default class TagParents extends Import {
  ...
  constructor(hydrus: Database, postgres: PoolClient, service: number) {
    ...
    this.inputQuery = `
      ...
      FROM tag_parents
      WHERE service_id=${service} AND status = ${ContentStatus.CURRENT} AND (child_tag_id, parent_tag_id) > (?, ?)
      ORDER BY child_tag_id, parent_tag_id
      LIMIT ?
    `;
    
    this.totalQuery = `SELECT count(1) FROM tag_parents WHERE service_id=${service} AND status=${ContentStatus.CURRENT}`;
  }
}
